...
void GenMeshTangents(Mesh *mesh)
{
    ...
    // Allocate and check tangents data
    if (mesh->tangents == NULL) mesh->tangents = (float *)RL_MALLOC(mesh->vertexCount*4*sizeof(float));
    if (mesh->tangents == NULL)
    {
        TRACELOG(LOG_WARNING, "MESH: Failed to allocate memory for tangents");
        return;
    }
    ...
    // Process all triangles of the mesh (triangleCount must be always valid)
